...
def pca_manual(data, n_comp=None):
    ...
    data = np.apply_along_axis(remove_mean, 0, data)
    ...
    covariance_matrix = data.T.dot(data)/(data.shape[0]-1)
#     covariance_matrix = np.cov(data.T)
    ...
    eigval_sorted_ids = np.argsort(eigval)[::-1]
    eigval_sorted = eigval[eigval_sorted_ids]
    eigvec_sorted = eigvec[:, eigval_sorted_ids]
    ...
    transformed_data = data.dot(K)


    # TODO: 7) Calculate the covariance matrix of the transformed data.
    covariance_matrix_transformed = np.cov(transformed_data.T)
    print("\nCOVARIANCE MATRIX OF THE TRANSFORMED DATA:")
    print(covariance_matrix_transformed)
